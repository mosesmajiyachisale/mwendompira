...
// ================================================================
  ...
  async function fetchData() {
    loading.value = true
    try {
      const { data } = await api.get(`/settings/providers/${providerId}`)

      provider_types.value = data?.provider_types || []
      qualifications.value = data?.qualifications || []
      seasons.value = data?.seasons || []

      const provider = data.selectedProvider || null
      if (!provider) {
        console.error('Provider not found')
        return
      }

      const person = provider.person
      ...
      formData.provider_code = provider.provider_code ?? null
      formData.preferred_name =provider.preferred_name ?? null
      formData.professional_qualification_id =provider.professional_qualification_id ?? null
      formData.provider_type_id =provider.provider_type_id ?? null
      formData.season_started =provider.season_started ?? null
...
async function saveChanges() {
  ...
  try {
    ...
    // const { data } = await api.put('/settings/providers', payload )
    const { data } = await api.put(`/settings/providers/${providerId}`,payload)
