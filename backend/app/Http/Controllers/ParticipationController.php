...
class ParticipationController extends Controller
{
    public function index(Request $request)
    {
        ...
        $participationsQuery = Participation::with(['season','tournament','team','sponsor','stadium']);
        $teamsQuery = Team::with('district')->orderBy('team_name', 'asc');

        if($season_id){
            $participationsQuery = $participationsQuery->where('season_id',$season_id);

            $teamsQuery = $teamsQuery->whereIn('id',function($query) use($season_id){
                $query->select('team_id')->from('participations')
